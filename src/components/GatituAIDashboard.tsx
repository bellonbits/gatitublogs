...
const GatituAIDashboard: React.FC = () => {
    ...
    return (
        <div className="flex h-screen w-full bg-[#09080b] text-[#d1d5db] overflow-hidden font-sans">
            {/* Sidebar */}
            <aside className="w-64 zyricon-glass border-r border-white/5 flex flex-col p-6 z-10">
                <div className="flex items-center space-x-3 mb-10">
                    <div className="w-8 h-8 rounded-full bg-purple-500 flex items-center justify-center shadow-[0_0_20px_rgba(168,85,247,0.3)]">
                        <Sparkles className="w-5 h-5 text-white" />
                    </div>
                    <span className="text-xl font-bold text-white tracking-tight">Gatitu AI</span>
                </div>
                    ...
                    {messages.length === 0 ? (
                        <div className="mt-20 text-center">
                            ...
                            >
                                ...
                                <h1 className="text-4xl md:text-5xl font-bold text-white mb-2 text-glow tracking-tight">Gatitu AI</h1>
                                <p className="text-gray-400 max-w-lg mx-auto">Ready to build something amazing today?</p>
                            </motion.div>
                        ...
                        <div className="w-full max-w-3xl space-y-8">
                            {messages.map((message) => (
                                <div key={message.id} className={`flex flex-col ${message.role === 'user' ? 'items-end' : 'items-start'}`}>
                                    <div className={`max-w-[85%] p-4 rounded-2xl ${message.role === 'user' ? 'bg-purple-600 text-white shadow-[0_0_15px_rgba(147,51,234,0.3)]' : 'zyricon-glass border border-white/5 text-[#d1d5db]'}`}>
...
const NavItem: React.FC<{ icon: any, label: string, active?: boolean }> = ({ icon: Icon, label, active }) => (
    <button className={`flex items-center space-x-3 w-full px-3 py-2 rounded-lg transition-all text-sm font-medium ${active ? 'bg-purple-500/10 text-purple-500 border border-purple-500/20 shadow-[0_0_15px_rgba(168,85,247,0.1)]' : 'text-gray-400 hover:bg-white/5 hover:text-white'}`}>
